...
CLI_NAME = Metadata.COMMAND_NAME
...
def _parse_parent(parent: str | None) -> list[str]:
    ...
    if any(token == "" for token in tokens):
        _abort(f"Invalid --parent '{parent}'. Use dot notation tokens with letters, numbers, underscore, and hyphen.")
...
def _new_command_meta_content(short_help: str, command_name: str) -> str:
    ...
    if META_TEMPLATE_TOKEN not in content:
        _abort("Template meta.yaml must include '{{SHORT_HELP}}' placeholder for --short-help.")
...
def _commands_dir() -> Path:
    ...
    return Metadata.USER_COMMANDS_DIR
