...
- **Dynamic command loading**: commands are discovered from the built-in `src/cli/commands/` tree and the per-user `$HOME/.<package name>/commands/` tree, then loaded lazily at runtime
- **Package management**: [uv](https://myclis.astral.sh/uv/) for fast dependency management
...
Install [uv](https://myclis.astral.sh/uv/).
...
`<package name>` comes from `Metadata.PACKAGE_NAME` in [`src/cli/utils/metadata.py`](src/cli/utils/metadata.py). User-defined commands are merged with built-in commands; when the same command path exists in both locations, the user-defined version wins.
...
By default, `admin new-command` writes to `$HOME/.<package name>/commands/`. Set an override if you want to target a different command tree during development.
